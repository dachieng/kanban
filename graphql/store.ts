...
const withRetry = async <T>(
  ...
  try {
    ...
    if (err instanceof GraphQLError || retries <= 0) throw err;
...
const getColumns = (): Promise<ColumnRecord[]> =>
  withRetry(() => db.select().from(columns).orderBy(asc(columns.order)));

const getTasksByColumn = (columnId: string): Promise<TaskRecord[]> =>
  withRetry(() =>
    db
      .select()
      .from(tasks)
...
const createColumn = (name: string): Promise<ColumnRecord> =>
  withRetry(async () => {
...
const renameColumn = (id: string, name: string): Promise<ColumnRecord> =>
  withRetry(async () => {
    const [column] = await db
      .update(columns)
      .set({ name })
...
const clearColumn = (id: string): Promise<ColumnRecord> =>
  withRetry(async () => {
...
const deleteColumn = (id: string): Promise<boolean> =>
  withRetry(async () => {
...
const createTask = (columnId: string, title: string): Promise<TaskRecord> =>
  withRetry(async () => {
...
const updateTask = (id: string, title: string): Promise<TaskRecord> =>
  withRetry(async () => {
    const [task] = await db
      .update(tasks)
      .set({ title })
